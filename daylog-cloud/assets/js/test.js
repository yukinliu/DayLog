    ...
    const els = {
      ...
      authGateSubtitle: document.querySelector("#authGateSubtitle"),
      ...
      switchToRegisterBtn: document.querySelector("#switchToRegisterBtn"),
      switchToLoginBtn: document.querySelector("#switchToLoginBtn"),
      ...
      accountBoundNote: document.querySelector("#accountBoundNote"),
      upgradeContactBtn: document.querySelector("#upgradeContactBtn"),
      proActivationSection: document.querySelector("#proActivationSection"),
      proActivationForm: document.querySelector("#proActivationForm"),
      ...
      proAccountDisplay: document.querySelector("#proAccountDisplay"),
    ...
    function init() {
      ...
      els.switchToRegisterBtn.addEventListener("click", () => setAuthMode("register"));
      els.switchToLoginBtn.addEventListener("click", () => setAuthMode("login"));
      els.forgotPasswordBtn.addEventListener("click", () => showAuthMessage("忘记密码暂时人工处理，请联系刘迷糊。", false));
      ...
      els.upgradeContactBtn.addEventListener("click", copyUpgradeContact);
      els.activateProBtn.addEventListener("click", activatePro);
      els.syncUploadBtn.addEventListener("click", uploadBackup);
      els.syncDownloadBtn.addEventListener("click", downloadBackup);
    ...
    function renderAccountDrawer() {
      ...
      const proActive = Boolean(account.proActive);
      els.accountStatus.classList.toggle("inactive", !proActive);
      els.accountStatus.innerHTML = loggedIn
        ? `<strong>${proActive ? "Pro 已激活" : "基础版 · Pro 未激活"}</strong><span>${escapeHtml(account.email)}</span>`
        : `<strong>未登录</strong>`;
      els.accountHint.textContent = loggedIn
        ? (proActive ? "当前账号已启用 Pro。备份和一键复制想法可用。" : "当前为基础版。你可以使用本地记录功能，激活 Pro 后可使用备份和一键复制想法。")
        : "未登录时，数据只保存在当前浏览器。请先注册或登录。";
      ...
      els.proActivationForm.hidden = !loggedIn || proActive;
      els.proActivePanel.hidden = !proActive;
      els.proAccountDisplay.textContent = account.email || "-";
      els.syncUploadBtn.disabled = !proActive;
      els.syncDownloadBtn.disabled = !proActive;
      els.syncUploadHint.textContent = account.lastUploadAt
        ...
        : "下一步接入云端备份";
      els.syncMessage.textContent = proActive ? "Pro 已激活，可使用数据备份。" : "Pro 激活后可使用数据备份。";
    }

    function setAuthMode(mode) {
      state.authMode = mode === "register" ? "register" : "login";
      const isRegister = state.authMode === "register";
      els.authGateTitle.textContent = isRegister ? "注册" : "登录";
      els.authGateSubtitle.textContent = isRegister ? "使用邮箱、密码和验证码创建 DayLog 账号。" : "使用邮箱和密码登录 DayLog。";
      els.authLoginTab.classList.toggle("active", !isRegister);
      els.authRegisterTab.classList.toggle("active", isRegister);
      els.authCodeField.hidden = !isRegister;
      els.loginAuthBtn.hidden = isRegister;
      els.verifyAuthCodeBtn.hidden = !isRegister;
      els.switchToRegisterBtn.hidden = isRegister;
      els.switchToLoginBtn.hidden = !isRegister;
    ...
    function renderAuthGate() {
      ...
      const mustLogin = !state.account.loggedIn;
      els.authGate.hidden = !mustLogin;
      if (mustLogin) {
    ...
    function setLoggedInAccount(data, email) {
      state.account = normalizeAccount({
        ...state.account,
        loggedIn: true,
        email: data.email || email,
        userId: data.userId || state.account.userId || "",
        token: data.token || state.account.token || "",
        proActive: Boolean(data.proActive || state.account.proActive),
        proActivatedAt: data.proActivatedAt || state.account.proActivatedAt || "",
    ...
    async function callAuthApi(action, payload = {}) {
      ...
      try {
        ...
        throw new Error("无法连接云函数。通常是云函数未开启 CORS，或接口尚未部署成功。");
      }
      let result = null;
      try {
    ...
    function setAuthBusy(isBusy) {
      ...
      els.activateProBtn.disabled = isBusy;
    }
    ...
    function showProMessage(text, positive = true) {
      if (!els.proMessage) return;
      els.proMessage.textContent = text;
      els.proMessage.style.color = positive ? "var(--green-text)" : "var(--muted)";
      clearTimeout(state.proMessageTimer);
      state.proMessageTimer = setTimeout(() => { els.proMessage.textContent = ""; }, 5000);
    }

    function copyUpgradeContact() {
      copyText("YKLSAC");
      showProMessage("微信号已复制，前往添加。", true);
    }

    async function activatePro() {
      const code = String(els.proCodeInput.value || "").trim();
      if (!state.account.loggedIn) {
        showProMessage("请先登录账号。", false);
        return;
      }
      if (!code) {
        showProMessage("请填写激活码。", false);
        return;
      }
      setAuthBusy(true);
      showProMessage("正在校验激活码...", true);
      try {
        const data = await callAuthApi("activatePro", {
        ...
        state.account = normalizeAccount({
          ...state.account,
          proActive: true,
          proActivatedAt: data.proActivatedAt || new Date().toISOString()
        });
        els.proCodeInput.value = "";
        saveData();
        renderAccountDrawer();
        render();
        showProMessage("Pro 已激活。", true);
      } catch (error) {
        showProMessage(error.message || "激活失败。", false);
      } finally {
        setAuthBusy(false);
      }
    }
    ...
    async function uploadBackup() {
      if (!state.account.proActive) {
        showProMessage("Pro 激活后可使用数据备份。", false);
        return;
      }
      const ok = window.confirm("确认上传本机数据备份吗？这会覆盖云端当前备份。");
      if (!ok) return;
      try {
        await callAuthApi("uploadBackup", {
          email: state.account.email,
          token: state.account.token,
        ...
        saveData();
        renderAccountDrawer();
        showProMessage("已上传数据备份。", true);
      } catch (error) {
        showProMessage(error.message || "上传失败。", false);
      }
    }
    ...
    async function downloadBackup() {
      ...
      const ok = window.confirm("下载最新数据会覆盖本机数据，确认继续吗？");
      if (!ok) return;
      try {
        const data = await callAuthApi("downloadBackup", {
          email: state.account.email,
          token: state.account.token
        });
        if (!data.backup || typeof data.backup !== "object") throw new Error("云端暂无可下载备份。");
        const currentAccount = state.account;
        Object.assign(state, normalizeData(data.backup));
        state.account = normalizeAccount({
          ...currentAccount,
          ...state.account,
          loggedIn: true,
        ...
        renderAccountDrawer();
        render();
        showProMessage("已下载最新数据。", true);
      } catch (error) {
        showProMessage(error.message || "下载失败。", false);
      }
    }
    ...
    function copyThoughtBundle() {
      if (!state.account.proActive) {
        showAuthMessage("Pro 激活后可用一键复制想法。", false);
        return;
      }
